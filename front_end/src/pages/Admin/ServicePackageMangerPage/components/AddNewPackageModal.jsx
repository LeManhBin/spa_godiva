...
import { fetchCreatePackage } from "../../../../api/package.api";
import PropTypes from 'prop-types';

const AddNewPackageModal = ({setIsOpen}) => {
    ...
    const addServiceMutation = useMutation({
        mutationFn: fetchCreatePackage
    })

    const handleSubmit = (e) => {
        e.preventDefault();
        addServiceMutation.mutate({...packageState, description: descriptionValue.split('/').map(item => item.trim())}, {
            onSuccess: (res) => {
                if(res.status === 200) {
