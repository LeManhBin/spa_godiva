...
import { fetchCreateStaff } from '../../../../api/staff.api';
import { toast } from 'react-toastify';
const AddStaffModel = ({setIsOpen}) => {
    ...
    const handleSubmit = (e) => {
        ...
        formData.append('name', staffState.name)
        formData.append('position', staffState.position)
        formData.append('avatar', staffState.avatar)
            ...
            <form className="p-4 md:p-5" onSubmit={handleSubmit}>
                <div className="grid gap-4 mb-4 grid-cols-2">
                    <div className="col-span-2">
                        ...
                        <input type="text" name="name" id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Nhập tên nhân viên" required value={staffState.name} onChange={handleOnChange} />
                    </div>
                    <div className="col-span-2">
                        <label htmlFor="position" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Vị trí</label>
                        <input type="text" name="position" id="position" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Nhập vị trí nhân viên" required value={staffState.position} onChange={handleOnChange} />
                    </div>
                    <div className="col-span-2">
                        <div className="flex gap-2.5 items-center">
                            <label htmlFor="upload" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Hình ảnh</label>
                        </div>
                        <input type="file" id='upload' onChange={(e) => setStaffState({...staffState, avatar: e.target.files[0]})}/>
                    </div>
                </div>
